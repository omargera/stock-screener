...
from tests.utils.fixtures import data_builder, mock_gateway
...
class TestSignalAccuracy:
    ...
    def test_resistance_breakout_entry_point(self, data_builder):
        ...
        assert signals.breakout.strength > 0.02, "Breakout strength should be significant"
        assert signals.breakout.strength < 0.10, "Breakout strength should be reasonable"
        
        # Should have volume confirmation
        assert signals.volume.signal, "Should detect volume spike"
        assert signals.volume.volume_ratio >= 2.5, "Volume should be significantly elevated"
    ...
    def test_moving_average_breakout_entry_point(self, data_builder):
        ...
        for i in range(40, 50):
            data.iloc[i, data.columns.get_loc('Close')] = 85.0 + np.random.uniform(-1, 1)
    ...
    def test_multiple_timeframe_consistency(self):
        ...
        configs = [
            ScreenerConfiguration(period="1mo", volume_spike_threshold=2.0, breakout_threshold=0.02),
            ScreenerConfiguration(period="3mo", volume_spike_threshold=2.0, breakout_threshold=0.02),
        ]
    ...
    def test_market_condition_detection(self, mock_gateway):
        ...
        for symbols, expected_condition_type in test_scenarios:
            ...
            if expected_condition_type == 'bullish':
                assert signal_pct >= 30 or condition in ['bullish', 'very_bullish'], \
                    f"Expected bullish market but got {condition} with {signal_pct}% signals"
            elif expected_condition_type == 'bearish':
                assert signal_pct <= 20 or condition in ['bearish', 'neutral'], \
    ...
    def test_entry_point_risk_assessment(self, data_builder):
        ...
        good_entry_data = (data_builder("TEST", 100.0)
                          .with_basic_data(60)
                          .with_resistance_at(110.0, 30, 55)
        ...
        assert quality['confidence'] > 0.6, "High-quality entry should have good confidence"
        ...
        poor_entry_data = (data_builder("TEST", 100.0)
                          .with_basic_data(60)
                          .build())
        
        # Weak breakout with low volume and high volatility
        poor_entry_data.iloc[55, poor_entry_data.columns.get_loc('Close')] = 101.0  # Minimal breakout
        poor_entry_data.iloc[55, poor_entry_data.columns.get_loc('Volume')] = int(
            poor_entry_data.iloc[55]['Volume'] * 0.8  # Low volume
        )
        
        enhanced_poor_data = self.technical_service.calculate_all_indicators(poor_entry_data)
        ...
        if poor_signals.has_any_signal:
            poor_quality = self.signal_service.analyze_signal_quality(poor_signals, enhanced_poor_data)
            assert poor_quality['confidence'] < 0.5, "Poor entry should have low confidence"
    ...
    def test_pattern_recognition_accuracy(self, data_builder):
        ...
        patterns = {
            'horizontal_resistance': {
                'setup': lambda d: d.with_resistance_at(110.0, 30, 55).with_breakout_on_day(56, 113.0, 2.5),
        ...
        for pattern_name, pattern_config in patterns.items():
            data = pattern_config['setup'](data_builder("TEST", 100.0).with_basic_data(60)).build()
            enhanced_data = self.technical_service.calculate_all_indicators(data)
            signals = self.signal_service.detect_all_signals(enhanced_data)
            
            if signals.breakout.signal:
                assert signals.breakout.signal_type.value == pattern_config['expected_type'], \
    ...
    def _create_ascending_triangle(self, builder):
        ...
        data.iloc[56, data.columns.get_loc('Volume')] = int(data.iloc[56]['Volume'] * 3.0)
    ...
    def _create_ma_crossover(self, builder):
        ...
        for i in range(40, 55):
            data.iloc[i, data.columns.get_loc('Close')] = 91.0 + np.random.uniform(-0.5, 0.5)  # Consolidation
        
        # Clear upward breakout
        for i in range(55, 60):
            data.iloc[i, data.columns.get_loc('Close')] = 92.0 + (i - 55) * 0.5
            if i == 56:
                data.iloc[i, data.columns.get_loc('Volume')] = int(data.iloc[i]['Volume'] * 2.0)
    ...
    def test_volume_pattern_analysis(self, data_builder):
        ...
        for pattern_name, volume_multiplier, should_detect in volume_patterns:
            ...
            if should_detect:
                assert signals.volume.signal, f"Should detect {pattern_name} volume pattern"
                assert signals.volume.volume_ratio >= 2.0, "Volume ratio should meet threshold"
            else:
                assert not signals.volume.signal, f"Should not detect {pattern_name} volume pattern" 