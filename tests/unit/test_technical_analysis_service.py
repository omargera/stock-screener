...
from tests.utils.fixtures import sample_stock_data, data_builder
...
class TestTechnicalAnalysisService:
    ...
    def test_calculate_all_indicators(self, sample_stock_data):
        ...
        expected_indicators = [
            ...
            'Volume_MA_20', 'Volume_ROC', 'OBV',
            'Price_Volatility', 'ATR',
            'Resistance', 'Support', 'Pivot',
            'Price_Change_Pct', 'Price_vs_SMA20', 'Price_vs_SMA50'
        ]
    ...
    def test_moving_averages_calculation(self, data_builder):
        ...
        assert abs(actual_sma_20 - expected_sma_20) < 0.01, \
        ...
        assert sma_values.iloc[-1] > sma_values.iloc[0], "SMA_20 should increase with increasing prices"
    ...
    def test_volume_indicators(self, data_builder):
        ...
        assert abs(actual_vol_ma - expected_vol_ma) < 1000, \
        ...
        assert obv_values.iloc[-1] > obv_values.iloc[0], "OBV should increase with rising prices"
    ...
    def test_get_latest_indicators(self, sample_stock_data):
        ...
        expected_keys = [
            'sma_20', 'sma_50', 'volume_ma_20', 'price_volatility',
            'resistance', 'support', 'atr', 'obv', 'price_change_pct',
    ...
    def test_obv_calculation_logic(self, data_builder):
        ...
        for i in range(len(expected_obv)):
            assert abs(obv[i] - expected_obv[i]) < 1, \
    ...
    def test_atr_calculation(self, data_builder):
        ...
        assert len(atr) > 0, "ATR should have values"
        assert all(8 <= a <= 12 for a in atr), \
